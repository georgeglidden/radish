...
function querytiles(query, metadata)
    ...
    leftidx = max(round(Int64, queryleft*resolution/width), 0)
    rightidx = round(Int64, queryright*resolution/width)
    querytop, querybottom = query.interval[2,:]
    topidx = max(round(Int64, querytop*resolution/height), 0)
    bottomidx = round(Int64, querybottom*resolution/height)
